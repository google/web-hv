...
function createJmuxPlayer(container) {
    ...
    ret.feed = function(data) {
        let firstFrame = jmuxer.kfCounter <= 0;
...
function createDecoderPlayer(container) {
    ...
    let canvas = ret.el.get(0);
    let ctx = canvas.getContext('2d');

    let lastFrame = null;

    async function onFrame(frame) {
        let isFirst = lastFrame == null;
        if (lastFrame != null) {
            lastFrame.close();
        }
        lastFrame = frame;
    ...
    function createConfig(header) {
        // Create description:
        let sps = header.sps[0].data;
        let pps = header.pps[0].data;
        return {
    ...
    ret.feed = function(data) {
        ...
        for (let i = 0; i < pendingFrames.length; i++) {
            ...
            switch (pendingFrames[i].type) {
                ...
                case SPS:
                case PPS: {
                    let data = pendingFrames[i].data;
                    let load = new Uint8Array(4 + data.byteLength);
...
let deviceMirrorAction = async function() {
    $("#main-progress").show();
    $("#device-list-content").empty().hide();
    $("#darkThemeSwitch").remove();
    $("#dmirrorview").removeClass("hide").removeClass("hidden");

    let player = window.VideoDecoder ? createDecoderPlayer("#dmirrorview .frame") : createJmuxPlayer("#dmirrorview .frame");
    ...
    async function startInputChannel() {
        ...
        player.el.mousedown(e => {
            ...
            let sendEvent = function(code, ev) {
              let x = Math.round((ev.pageX - offsetX) * width);
              let y = Math.round((ev.pageY - offsetY) * height);
        ...
        let keyMap = {};
        let addKeyMap = function(jCode, aCode, length) {
        ...
        $(document).bind("keydown keyup", function(e) {
            ...
            let response = `ke:${e.type == 'keydown' ? 'd' : 'u'}:${e.altKey ? 1 : 0}:${e.ctrlKey ? 1 : 0}:${e.metaKey ? 1 : 0}:${e.shiftKey ? 1 : 0}:${code}\n`;
    ...
    player.onMetadata = function() {
        let maxW = $("#dmirrorview").width() - 40;
        let maxH = $("#dmirrorview").height() - 40;
        let s = Math.max(player.videoWidth() / maxW, player.videoHeight() / maxH);
    ...
    // Get device size
    let sizeArg = "";
    let size = /\b(\d+)x(\d+)\b/.exec(await adbDevice.shellCommand("wm size"));
    if (size) {
        let w = Math.round(parseInt(size[1]) / 2);
        let h = Math.round(parseInt(size[2]) / 2);
        sizeArg = ` --size=${w}x${h}`;
        videoSizeFactor = 2;
    }
    ...
    function connectStream() {
        ...
        let stream = adbDevice.openStream(`shell:screenrecord ${sizeArg} --output-format=h264 - `);
